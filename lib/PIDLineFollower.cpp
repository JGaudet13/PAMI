...
PIDLineFollower::PIDLineFollower(Motor* right_motor, Motor* left_motor, SensingThread* sensor_thread)
  ...
  pid_line_follower = new PID(0.8, 0.1, 0.5);  // Gains par défaut pour suivi de ligne
  pid_motor_right = new PID(1.5, 0.3, 0.2);    // Gains par défaut pour moteur droit
  pid_motor_left = new PID(1.5, 0.3, 0.2);     // Gains par défaut pour moteur gauche
}
...
void PIDLineFollower::followLine() {
  ...
  // ===== ÉTAPE 2: CONTRÔLE PROPORTIONNEL SIMPLE =====
  double Kp = 0.02; // Gain proportionnel simple
  double correction = Kp * info.LS_error;
